...
(*get fulu/non fulu tiles of a player*)
...
(*set a list of tiles as fulu tiles*)                                                                    
let rec set_fulu_helper (remain: Tile.t list) (to_set_fulu: Tile.t list) (passed: Tile.t list)= 
  ...
  | x::xs ->  if Tile.check_equal x (List.nth_exn remain 0)
              then (Tile.change_fulu x;
                    if List.length remain >= 1 
                    then (set_fulu_helper (List.sub remain ~pos:1 ~len:(List.length remain -1)) 
                            xs
                            passed@[x])
...
(*put the to_put tile list into our ron info*)
...
(*when user chii pong kan, update the according info , set user as not mensei*)
...
(*construct a ron info with the shun_list, pong_list, kan_list, pair*)
...
(*check if two ron info are the same*)
...
(*remove the duplicate ron_info. Only keep one copy of ron_info if many ron infos are duplicate*)
...
(*check if the list of tiles are all tanyao*)
let tanyao_check (t_list:Tile.t list) = 
  ...
  !all_tanyao 
(*check if the hand only contains tile of one kind of man/suo/pin. Must contain wind or dragon for rest of hand*)
...
(*check if the hand only contains tile of one kind of man/suo/pin. *)
let qing_yisu_check (t_list:Tile.t list) = 
  ...
  else if (List.length pin_list = 0 ) && (List.length suo_list = 0) && (List.length dragon_list = 0) && (List.length wind_list = 0) then true
  else if (List.length man_list = 0 ) && (List.length suo_list = 0) && (List.length dragon_list = 0) && (List.length wind_list = 0) then true
  else false
  
(*check if the hand only contains wind and dragon tiles*)
...
(*check if a dragon triplet is present in hand*)
...
(*check if a list has one pair of same tile left*)
...
(*delete a list of tile from a list, and return them as tuple*)
let operate (t_list:Tile.t list ) (x:Tile.t list)=
  let new_t_list = Tile.delete_list_of_tile t_list x in
  (new_t_list,x)
(*add a tile list (shun/pong/kan) to ron_info list*)
...
let rec check_ron_helper (t_list:Tile.t list) = 
  ...
  List.iter t_list ~f:(fun x-> let operate_list = Tile.find_all_ke_shun t_list x in
                               if (List.is_empty operate_list) 
                               then (
                                 if one_pair_left ( t_list) then (ron_ref := true;
                                                                  ron_info := (ron_info_constructor [] [] [] t_list)::[];
                               ))
                               else 
                                 List.iter operate_list 
                                   ~f:(fun y -> let (new_t_list,info_ele) = operate t_list y in
  ...
  (!ron_ref,!ron_info)
(*check if the hand satisfies requirement for ron. Uses dynamic programming*)
let check_ron (t_list_have_fulu:Tile.t list) =
  let ron_info = ref [] in
  let ron_ref = ref false in
  let t_list = Tile.get_all_non_fulu t_list_have_fulu in 
  List.iter t_list ~f:(fun x-> let operate_list = Tile.find_all_ke_shun t_list x in
                               if (List.is_empty operate_list) then (if one_pair_left ( t_list) then (ron_ref := true;
                                                                                                      ron_info := (ron_info_constructor [] [] [] t_list)::[];
                                                                    ))
                               else
                                 List.iter operate_list 
...
(*check if all mian zu are shun, and are all non-fulu*)
...
let has_dai_san_gan (info: ron_info) = 
  if (List.length info.pong_list + List.length info.kan_list < 3 ) then false
        ...
        List.iter info.pong_list ~f:(fun x -> let pos_ele =  List.nth_exn x 0 in
                                              if Tile.check_equal pos_ele (Tile.construct_fake Dragon 1) then is_Bai := true;
                                              if Tile.check_equal pos_ele (Tile.construct_fake Dragon 2) then is_Chong := true;
                                              if Tile.check_equal pos_ele (Tile.construct_fake Dragon 3 ) then is_Fa := true;
          );
...
(*check if hand only contains lao tou tiles : Man/Suo/Pin 1/9*)
let is_qing_lao_tou (info: ron_info) = 
  let laotou = ref true in 
  if (List.length info.pong_list + List.length info.kan_list < 4) then false
  else (List.iter info.pong_list ~f:(fun x -> List.iter x ~f:(fun y -> if not(Tile.check_laotou y) 
                                                                       then laotou := false ));
        !laotou)
  
let append_missing (fong: bool list)  = 
  let missing_fong = ref 0 in
  let is_missing = List.foldi fong ~init:[] ~f:(fun i acc x -> if (not x) 
                                                               then (missing_fong := i;
                                                                     x::acc)
                                                               else acc) in
  ...
  | 1 -> (true,!missing_fong) 
  | _ -> (false,-88)
(*check if hand has 3 wind triplet or kan, and has the missing wind triplet as the pair*)
let is_Xiao_su_xi (info: ron_info) = 
  ...
  List.iter info.pong_list ~f:(fun x -> let pos_ele = List.nth_exn x 0 in 
                                        if (Tile.check_equal pos_ele (Tile.construct_fake Wind 1)) then tong := true;
                                        if (Tile.check_equal pos_ele (Tile.construct_fake Wind 2)) then nan := true;
                                        if (Tile.check_equal pos_ele (Tile.construct_fake Wind 3)) then xia := true;
                                        if (Tile.check_equal pos_ele (Tile.construct_fake Wind 4)) then pei := true;
    );
  List.iter info.kan_list ~f:(fun x -> let pos_ele = List.nth_exn x 0 in
                                       if (Tile.check_equal pos_ele (Tile.construct_fake Wind 1)) then tong := true;
                                       if (Tile.check_equal pos_ele (Tile.construct_fake Wind 2)) then nan := true;
  ...
  | (false,_) -> false 
(*check if hand has 4 wind triplet or kan.*)
let is_Dai_su_xi (info: ron_info) = 
  ...
  !tong && !nan && !xia && !pei
(*check if two lists are non fulu and equal*)
...
(*check if ron_info has two shun that are identical and non fulu*)
...
(*check if ron_info has two pairs of two shun that are identical and non fulu*)
...
(*check three tile lists : if the numbers of 0th, 1st, 2nd position of the list are all identical*)
...
(*check if a ron_info has 3 shun such that 0th 1st 2nd position of those shuns have identical number *)
let san_su_tong_shun (info: ron_info) = 
  ...
  | 4 -> let fst_shun = List.nth_exn info.shun_list 0 in
         ...
         let thr_shun = List.nth_exn info.shun_list 2 in
         let for_shun  = List.nth_exn info.shun_list 3 in 
         check_all_tiles_number_eq fst_shun snd_shun thr_shun || check_all_tiles_number_eq fst_shun snd_shun for_shun ||
           check_all_tiles_number_eq fst_shun thr_shun for_shun || check_all_tiles_number_eq snd_shun thr_shun for_shun 
  | _ -> false

(*check if a ron_info has 3 pong such that 0th 1st 2nd position of those shuns have identical number *)
...
(*check if a ron_info has 3 non fulu triplets*)  
let san_an_ke (info: ron_info) = 
  ...
  | _ -> false
(*check if a ron_info has 4 non fulu triplets*)
...
(*check if a hand contains tiles of a kind from num 1 to 9*)
let yi_cu (info: ron_info) = 
  ...
  | 4 -> let first_shun = List.nth_exn info.shun_list 0 in 
         ...
         (compose_one_to_nine first_shun second_shun third_shun || compose_one_to_nine first_shun second_shun fourth_shun ||
            compose_one_to_nine first_shun third_shun fourth_shun || compose_one_to_nine second_shun third_shun fourth_shun)
  | _ -> false
(*check if a hand is consisted of shun with one tile being lao tou and pong with tiles being yao 9 tile*)
let han_chan_dai (info: ron_info) = 
  ...
  let pong_contains_nonyao = List.fold all_pong ~init:true ~f:(fun acc x -> Tile.contain_yao_list x && acc) in 
  let all_kan = info.kan_list in 
  let kan_contains_nonyao = List.fold all_kan ~init:true ~f:(fun acc x -> Tile.contain_yao_list x && acc) in 
  let pair_contains_nonyao = Tile.contain_yao_list info.pair_ron in 
  shun_contains_laotou && pong_contains_nonyao && kan_contains_nonyao && pair_contains_nonyao
(*check if a hand is consisted of pong with tiles being yao 9 tile*)
let han_lao_tou (info: ron_info) = 
  if (List.length info.shun_list > 0 ) then false 
  else (let all_pong = info.pong_list in
        let pong_contains_yao = List.fold all_pong ~init:true ~f:(fun acc x -> Tile.contain_yao_list x && acc) in
        let all_kan = info.kan_list in
        let kan_contains_yao = List.fold all_kan ~init:true ~f:(fun acc x -> Tile.contain_yao_list x && acc) in
        let pair_contains_yao = Tile.contain_yao_list info.pair_ron in
        pong_contains_yao && kan_contains_yao && pair_contains_yao
       )
(*check if a hand contains 2 dragon triplet, and the remaining one as pair*)
...
(*if a kind only consist of shun that contains lao tou tiles*)
...
(*check if all tiles are of green color*)
...
type yaku = Riichi | Tanyao | Hunyise | Qinyise | Chankan
            ...
            | Hanlaotou | Xiosangan | Greenyise | Sianke | Tuitui | None
...
[@@@coverage off]
(*calculate the number of yaku man a hand satisfies. When a yaku mman is reached, we dont check non yaku man*)
let cal_yaku_man (hand : ron_info) (t : Tile.t list) = 
  ...
  let () = (match green_yi_su t with 
            ...
            | false -> ()) in
  !all_yaku_man
   (*calculate the number of non yaku man a hand satisfies*)
...
(*calculate the maximum yaku a list of ron_info satisfies. Yakus can accumulate*)
let cal_yaku (t_list:Tile.t list) (p:player) (info:ron_info list) = 
  let yaku_list = List.map info ~f:(fun x ->let yaku_man_list = cal_yaku_man x t_list in
...
(*get the tiles you need to ron, when you riichi*)

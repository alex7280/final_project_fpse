...
(*constructor of real tiles and test tiles*)
...
(*generate all 136 tiles of pai Shan*)
let generate_game_tiles _ = 
  ...
  List.fold [1;2;3] ~init:generated_wind_tiles
    ~f:(fun acc x-> let t = class_constructor !id_gen Dragon x false false false in
                    ...
                    id_gen := !id_gen + 4;
                    acc @[t;t1;t2;t3])
(*check if a tile is pin/suo/man*)
...
(*check if is wind or dragon*)
let is_wind_dragon (to_check:tile) = 
  match to_check.kind with 
  | Dragon -> true
  | Wind -> true
  | _ -> false 
(*check if is green tile*)
...
let check_yao (to_check:tile) = 
  not (check_non_yao to_check)
...
let contain_yao_list (to_check:tile list) = 
  List.fold to_check ~init:false ~f:(fun acc x -> check_yao x || acc)
    
    [@@@coverage off]
(*the to string functions/print functions are marked as coverage off*)
(*string rep of tiles*)
...
let check_shun (first:tile) (second:tile) (third:tile):bool = 
  let adj small big = if small.number = big.number -1 then true
            else false in
  (adj first second) && (adj second third)
...
(*sort a hand from man- Pin- Suo - Dragon - Wind *)
...
(* get non fulu or fulu tiles of tile list*)
...
let rec delete_one_tile (l:tile list) (to_del:tile) (acc:tile list): tile list = 
  ...
  | ele::ele_list -> if (equal_kind ele.kind to_del.kind) && (ele.number = to_del.number) && not (ele.fulu)
                     then sort_hand (acc@ele_list)
                     else delete_one_tile ele_list to_del acc@[ele]
(*delete list of tiles*)
...
(*get all shun that the to_set tile can form with the original list l*)
...
(*this checks which tile list is bigger, depending on the comparision value between tiles*)
...
(* find all the available shun/ke /kan that the to_set tile can form with the original list l*)
...
(*call generate man, pin, suo ,dragon,wind respectively to generate all tiles*)
